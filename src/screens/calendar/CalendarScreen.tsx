...
export default function CalendarScreen({ navigation }: { navigation: any }) {
  ...
  const allOccasions: OccasionEntry[] = useMemo(() => {
    const list: OccasionEntry[] = [];
  ...
  const eventsByDay: Record<number, OccasionEntry[]> = {};
  ...
  const groupedByDate = upcomingSorted.reduce<Record<string, OccasionEntry[]>>((acc, o) => {
  ...
  const goToPerson = (personId: string) =>
    navigation.navigate('People', { screen: 'PersonProfile', params: { personId } });
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.weekdayRow}>
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
              <Text key={i} style={[styles.weekdayLabel, { color: theme.colors.textSecondary }]}>
                {d}
              </Text>
            ))}
          </View>
                ...
                <View key={idx} style={styles.cell}>
                  {day ? (
                    <View
                      style={[
                        styles.dayCircle,
                        isToday ? { backgroundColor: theme.colors.primary } : undefined,
                      ]}
                    >
                      <Text style={{ color: isToday ? '#fff' : theme.colors.text, fontSize: 13 }}>{day}</Text>
                      {hasEvent ? (
                        <View style={[styles.eventDot, { backgroundColor: isToday ? '#fff' : theme.colors.accent }]} />
                      ) : null}
                    </View>
                  ) : null}
                </View>
          ...
          {occasionsInMonth.length === 0 ? (
            <EmptyState emoji="🗓️" title="No events this month" />
          ) : (
            occasionsInMonth.map((o, idx) => (
              <Card key={idx} onPress={() => goToPerson(o.personId)} style={{ marginBottom: 10 }}>
      ...
      {mode === 'Week' && (
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {upcomingSorted.filter((o) => daysUntil(o.date) <= 7).length === 0 ? (
            <EmptyState emoji="✅" title="Nothing this week" subtitle="Enjoy the calm before the next celebration." />
          ) : null}
        </ScrollView>
      )}
