...
export default function HomeScreen({ navigation }: { navigation: any }) {
  ...
  const upcoming = useMemo(() => {
    const list: { personId: string; personName: string; emoji: string; color: string; label: string; date: string; days: number }[] = [];
    people.forEach((p) => {
        ...
        return (
          <Card key={r.id} style={styles.reminderCard}>
            <View style={styles.reminderRow}>
              <Text style={styles.reminderEmoji}>{person?.avatarEmoji ?? '🎉'}</Text>
