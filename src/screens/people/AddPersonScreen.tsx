...
const interestOptions = [
  'Travel', 'Books', 'Gaming', 'Music', 'Sports', 'Cooking', 'Fashion',
  'Tech', 'Art', 'Fitness', 'Movies', 'Gardening', 'Coffee', 'Wine',
];

const steps = ['Basics', 'Dates', 'Interests', 'Budget', 'Details', 'Review'];

export default function AddPersonScreen({ navigation }: { navigation: any }) {
  ...
  const save = () => {
    const newPerson: Person = {
      ...
      occasions: birthday
        ? [{ id: `o-${Date.now()}`, type: 'Birthday', label: `${name || 'New Person'}'s Birthday`, date: birthday, recurring: true }]
        : [],
      interests,
      sizes: sizes || undefined,
      budgetMin: budgetMin ? Number(budgetMin) : undefined,
      budgetMax: budgetMax ? Number(budgetMax) : undefined,
      wishlistUrl: wishlistUrl || undefined,
        ...
        {step === 0 && (
          <View>
            ...
            <View style={styles.rowWrap}>
              {avatarEmojis.map((e) => (
                <Pressable key={e} onPress={() => setAvatarEmoji(e)} style={styles.emojiOption}>
