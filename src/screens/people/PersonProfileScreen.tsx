...
export default function PersonProfileScreen({ route, navigation }: { route: any; navigation: any }) {
      ...
      <View style={styles.content}>
        {tab === 'Overview' && (
          <View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {(relatedSuggestions.length ? relatedSuggestions : giftSuggestions.slice(0, 3)).map((g) => (
                <GiftSuggestionCard
                  key={g.id}
                  name={g.name}
                  category={g.category}
            ...
            <View style={{ marginTop: 16 }}>
              ...
              <View style={styles.chipsRow}>
                {person.interests.map((i) => (
                  <View key={i} style={[styles.interestChip, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
        ...
        {tab === 'Occasions' && (
          <View>
            {sortedOccasions.length === 0 ? (
              <EmptyState emoji="📅" title="No occasions yet" subtitle="Add a birthday or anniversary to get reminders." />
            ) : (
        ...
        {tab === 'Gifts' && (
          <View>
            ...
            {history.length === 0 ? (
              <Text style={{ color: theme.colors.textSecondary, marginBottom: 16 }}>No gifts recorded yet.</Text>
            ) : (
              history.map((h) => (
            ...
            {wishlistItems.length === 0 ? (
              <Text style={{ color: theme.colors.textSecondary }}>Nothing on the wishlist yet.</Text>
            ) : (
              wishlistItems.map((w) => (
