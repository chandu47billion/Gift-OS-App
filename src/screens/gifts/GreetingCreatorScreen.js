...
const tones = ["Heartfelt", "Funny", "Formal", "Short"];
function generateGreetings(name, tone) {
  switch (tone) {
    case "Heartfelt":
      return [
        `Dear ${name}, wishing you a day as wonderful and warm as the joy you bring to everyone around you. Happy celebration! \u{1F49B}`,
    ...
    case "Funny":
      return [
        `Happy Birthday ${name}! You're not getting older, you're just becoming a classic. \u{1F604}`,
    ...
    case "Short":
      return [
        `Happy day, ${name}! \u{1F389}`,
        `Cheers to you, ${name}!`,
        `Congrats, ${name} \u2014 enjoy!`
      ];
  }
}
function GreetingCreatorScreen({ navigation, route }) {
  const theme = useTheme();
  const { state } = useAppStore();
  const [personId, setPersonId] = useState(route?.params?.personId ?? state.people[0]?.id ?? "");
...
export {
  GreetingCreatorScreen as default
};
