...
function GiftHistoryScreen({ navigation }) {
  ...
  const filtered = useMemo(() => {
    return [...state.giftHistory].filter((h) => personFilter === "All" || h.personId === personFilter).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  />
    ...
    renderItem={({ item }) => <Card style={{ marginBottom: 12 }}>
